...
#include "gen/antlr/ParadoxFastLexer.h"
#include "gen/antlr/ParadoxFastParser.h"
#include "gen/antlr/ParadoxFastBaseListener.h"

class ConsoleWritingListener : public ParadoxFastBaseListener {
public:
...
class AssignmentListener : public ParadoxFastBaseListener {
public:
    AssignmentListener(bool quiet = true): quiet_(quiet) {}

    virtual void enterAssignment(ParadoxFastParser::AssignmentContext *ctx) override {
    ...
    virtual void exitAssignment(__attribute__((unused)) ParadoxFastParser::AssignmentContext *ctx) override {
        if (!quiet_) {
...
private:
    ...
    std::string _get_assignment_name(ParadoxFastParser::AssignmentContext *ctx) {
        if (!ctx) {
    ...
    std::string _get_assignment_name(ParadoxFastParser::FieldContext *field) {
        if (field->symbol()) {
    ...
    std::string _get_assignment_name(ParadoxFastParser::SymbolContext *symbol) {
        if (symbol->string()) {
    ...
    std::string _get_assignment_name(ParadoxFastParser::StringContext *string) {
        if (string->DSTRING()) {
    ...
    std::string _get_assignment_name(ParadoxFastParser::VariableContext *variable) {
        if (variable->SYMBOL()) {
